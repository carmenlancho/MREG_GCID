...
Una de las aplicaciones más útiles del Cp de Mallows es su visualización gráfica para identificar el mejor modelo. El siguiente ejemplo muestra cómo crear un gráfico de Cp que facilita la interpretación y selección del mejor modelo.
...
Como se observa, el estadístico Cp disminuye drásticamente al pasar de uno a dos predictores. El modelo con **`r mejor_cp_idx` variables** es el seleccionado como el mejor porque alcanza el valor de Cp más bajo de todos los candidatos, con un valor de **`r round(subset_summary$cp[mejor_cp_idx], 2)`**.

El análisis sugiere que el modelo más parsimonioso y con el mejor rendimiento predictivo se compone de las siguientes variables: **`r paste(variables_mejor_modelo_cp, collapse = ", ")`**. Añadir más predictores más allá de este punto no mejora el modelo; de hecho, el valor de Cp comienza a aumentar, lo que indica que estamos añadiendo una complejidad innecesaria y empezando a sobreajustar los datos.
...
El método de selección exhaustiva aplicado al conjunto de datos `mtcars` identifica el modelo que maximiza el R² ajustado. Este modelo alcanza un **R² ajustado de `r round(mejor_r2_adj, 4)`**, utilizando **`r variables_seleccionadas` variables** del total de **`r total_variables` variables** predictoras disponibles. Esta selección representa un equilibrio entre la capacidad explicativa del modelo y la penalización por complejidad, demostrando cómo la evaluación exhaustiva puede identificar el subconjunto de variables que mejor explica la variabilidad en el consumo de combustible.
:::
...
La elección de $\lambda$ se determina generalmente mediante **validación cruzada**.

::: {.callout-caution title="Aviso"}
Los detalles de la validación cruzada son tratados en la asignatura de Aprendizaje Automático I.
:::
...
# Seleccionar lambda con validación cruzada
cv_ridge <- cv.glmnet(X, Y, alpha = 0)
lambda_optimo <- cv_ridge$lambda.min  # Mejor valor de lambda

print(lambda_optimo)

# Ajustar modelo final con el lambda seleccionado
...
Al igual que en el método *Ridge*, la selección de $\lambda$ se realiza generalmente mediante **validación cruzada**.
...
-   **Simplicidad e interpretabilidad:** Un modelo con menos variables es más fácil de interpretar y aplicar en la práctica.
...
lambda_sel <- cv_lasso$lambda.min  # Mejor valor de lambda

print(lambda_sel)

# Ajustar modelo final con el lambda seleccionado
modelo_lasso_final <- glmnet(X, Y, alpha = 1, lambda = lambda_sel)
...
lambda_sel <- cv_elastic_net$lambda.min  # Mejor valor de lambda

print(lambda_sel)
# Ajustar modelo final con el lambda seleccionado
modelo_elastic_final <- glmnet(X, Y, alpha = 0.5, lambda = lambda_sel)
...
-   Requiere la selección de hiperparámetros ($\lambda$ y $\alpha$), por lo que debe usarse validación cruzada para encontrar la mejor combinación.
...
Una vez que usamos una estrategia de validación para generar predicciones sobre datos no vistos, necesitamos una "nota" para cuantificar cómo de buenos fueron esos pronósticos. Aquí es donde entran las métricas de error.
...
Usaremos la función `train()` del paquete `caret`, que es una herramienta potente para ajustar y validar modelos. Configuraremos una **validación cruzada de 10 particiones (10-fold CV)** para estimar el error de predicción (RMSE) de nuestros dos modelos.
