...
Con esto, tenemos que $$\hat{\mathbf{y}} = \mathbf{X}\hat{\boldsymbol{\beta}} = \underbrace{\mathbf{X}(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T}_{\mathbf{H}}\hat{\mathbf{y}} = \mathbf{H} \hat{\mathbf{y}}$$
...
-   **Error estándar residual**
    ($\hat{\sigma}$ = `r round(summary(modelo)$sigma, 0)` euros)
...
-   **Habitaciones** (`r coefs[3]` €): Cada habitación adicional
    incrementa el precio en `r coefs[3]` euros en promedio,
...
-   **Garaje** (`r coefs[6]` €): Las viviendas con garaje cuestan
    `r coefs[6]` euros más que las que no tienen, **en promedio y
...
Tema 1: la **construcción de intervalos de confianza para los
parámetros**.
...
Aquí tienes una versión revisada y ampliada de la sección **"Inferencia
estadística en el modelo múltiple"** que incluye explícitamente los
intervalos de confianza, manteniendo el flujo que hemos construido.
Puedes reemplazar la sección anterior por esta.
...
Llamemos a estos residuos $e_{Y|X_{-j}}$. 2. Se calculan los residuos de
la regresión de $X_j$ en función de todos los demás predictores.
Llamemos a estos residuos $e_{X_j|X_{-j}}$. 3. Se grafica $e_{Y|X_{-j}}$
