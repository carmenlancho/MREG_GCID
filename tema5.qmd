...
-   **Flexibilidad:** Los GLM permiten modelar diferentes tipos de variables dependientes, lo que amplía el rango de problemas que se pueden abordar.
...
Esto significa que la probabilidad de que ocurra el evento es 4 veces la probabilidad de que no ocurra. Equivalentemente, se dice que los odds son 4:1 a favor del evento.

El **odds ratio (OR)** mide cómo cambian los odds cuando una variable independiente aumenta en una unidad. En regresión logística, el OR asociado al coeficiente ($\beta_j$) se calcula como:
...
-   Si **OR \> 1**, los odds del evento aumentan a medida que aumenta la variable independiente.
-   Si **OR \< 1**, los odds del evento disminuyen a medida que aumenta la variable independiente.
-   Si **OR = 1**, no hay efecto.

Por ejemplo, si $e^{\beta_j}=1.5$, entonces un aumento de una unidad en $x_j$ multiplica los odds por 1.5. Esto no significa necesariamente que la probabilidad aumente en un 50%, porque el cambio en probabilidad depende del valor inicial de $p$.
...
La deviance mide cómo de bien se ajusta un modelo logístico en comparación con el modelo saturado, es decir, el modelo que reproduce perfectamente los datos observados. En la práctica, también suele compararse la deviance del modelo ajustado con la deviance del modelo nulo, que solo incluye intercepto y predice la misma probabilidad para todas las observaciones.
